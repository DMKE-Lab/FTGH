...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        x = x.view(batch_size, -1, self.num_heads * self.att_size).squeeze(1)
...
class Attention_Module(nn.Module):
    def __init__(self, args, embed, num_symbols, embedding_size, use_pretrain=True, finetune=True, dropout_rate=0.3):
        ...
        self.rel_w = nn.Bilinear(self.embedding_size, self.embedding_size, 2* self.embedding_size, bias=False)
    ...
    def forward(self, entity_pairs, entity_meta):
        ...
        entity = self.dropout(self.symbol_emb(entity_pairs))
        ...
        rel_emb_forward, rel_emb_backward = torch.split(rel_emb, entity_left.size(-1), dim=-1)
