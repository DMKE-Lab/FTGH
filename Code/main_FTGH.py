...
if __name__ == '__main__':
    ...
    dataset['dev_tasks'] = json.load(open(data_dir['dev_tasks.json']))
    ...
    dataset['rel2id'] = json.load(open(data_dir['rel2ids_shuffle']))
    dataset['time2id'] = json.load(open(data_dir['timeid2.json']))

    if params['data_form'] == 'Pre-Train':
        print('loading embedding ... ...')
        dataset['ent2emb'] = np.loadtxt(params['data_path']+'/ent2ids')
        dataset['rel2emb'] = np.loadtxt(params['data_path']+'/relation2ids')
        dataset['time2emb'] = np.loadtxt(params['data_path'] + '/timeid2vec.json')
