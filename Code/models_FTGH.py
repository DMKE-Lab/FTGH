...
class LSTM_attn(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # # todo 打算从这里加入时间的嵌入表示
        # rel_embed = self.rel_embed.expand_as(inputs)
        #
        # #     todo 添加
        # # time_embed = tem.expand_as(left)  ###
        #
        # left = inputs.unsqueeze(1)
        # right = inputs.unsqueeze(1)
        # # todo 添加
        # # tem = tem.unsqueeze(1)
...
class MetaR(nn.Module):
    ...
    def neighbor_encoder(self, connections, num_neighbors, istest,query, key, value):
        ...
        # attn1,connections1=Attention_Module.SPD_attention(entity_self,relations)
        # attn2,connections2=Attention_Module.SPD_attention(entity_self, ent_embeds)
        #这里修改
        entself_embeds = self.dropout(self.symbol_emb(entity_self))

        concat_embeds = torch.cat((rel_embeds, ent_embeds), dim=-1)
        # atten3=atten1*atten2
        ...
        #这里做了修改
        entself_embeds=entself_embeds[:,1,:].squeeze(-1)
    ...
    def forward(self, task, iseval=False, curr_rel='', support_meta=None, istest=False):
        ...
        if iseval and curr_rel != '' and curr_rel in self.rel_q_sharing.keys():
            ...
            if not self.abla:
                ...
                p_score, n_score = self.embedding_learner(sup_neg_e1, sup_neg_e2, rel_s, few, norm_vector)
